...
class WrapperProblem:
    ...
    def evaluate(self, X, *args, **kwargs):
        ...
        for i, x in enumerate(X):
            ...
            self.logger.call(
                self.create_info(
                    pymoo_evaluate_result["F"][i][0] + self.ideal_point()[0], x
                )
            )
...
def run_experiment():
    ...
    for idx, problem_name in enumerate(problems):
        ...
        wrapper_problem = WrapperProblem(
            problem,
            algorithm_name=f"NSGA2",
            algorithm_info=f"{popsize}",
            exp_attributes=exp_attrs,
            fid=idx,
            folder_name=f"NSGA_{popsize}",
        )
        algorithm = NSGA2(pop_size=popsize)
        for i in range(5):
            res = minimize(
                wrapper_problem, algorithm, ("n_eval", 2000), seed=i, verbose=True
            )
            ...
            wrapper_problem.evaluate(
                res.X, return_values_of=["F", "G", "H"], return_as_dictionary=True
            )
            print(wrapper_problem.num_evaluations)
            wrapper_problem.reset()
        wrapper_problem = WrapperProblem(
            problem,
            algorithm_name=f"SMS-EMOA",
            algorithm_info=f"{popsize}",
            exp_attributes=exp_attrs,
            fid=idx,
            folder_name=f"SMSEMOA_{popsize}",
        )
