...
class TestFixedTarget(unittest.TestCase):
    ...
    def test_custom_op(self):
        ...
        result = aggregate_running_time(
            self.df, custom_op=my_sum, return_as_pandas=False
        )
    ...
    def test_with_f_min_f_max(self):
        result = aggregate_running_time(
            self.df, f_min=0.2, f_max=0.5, return_as_pandas=False
        )
    ...
    def test_with_different_free_variables(self):
        result = aggregate_running_time(
            self.df, free_vars=["algorithm_name", "data_id"], return_as_pandas=False
        )
    ...
    def test_success_ratio_and_count(self):
        ...
        df = self.df.with_columns(
            [
                pl.when(pl.col("evaluations") == 3)
                .then(float("nan"))
                .otherwise(pl.col("evaluations"))
                .alias("evaluations")
            ]
        )
