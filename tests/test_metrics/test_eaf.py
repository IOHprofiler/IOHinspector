...
class TestGetDiscritizedEAF(unittest.TestCase):
    ...
    def test_basic_single_data_id(self):
        ...
        self.assertTrue(
            result[self.data["evaluations"].to_list()]
            .map(lambda x: x in [1, 0])
            .all()
            .all()
        )
    ...
    def test_basic_multi_data_id(self):
        ...
        self.assertTrue(
            result[self.multi_data["evaluations"].to_list()]
            .map(lambda x: x in [1, 0.5, 0])
            .all()
            .all()
        )
        self.assertEqual(result[1].tolist()[-1], 0)
    ...
    def test_scale_eval_log_and_f_log(self):
        result = get_discritized_eaf_single_objective(
            self.data, scale_f_log=False, scale_eval_log=False
        )
...
class TestGetEAFData(unittest.TestCase):
    ...
    def test_basic_with_simple_data(self):
        ...
        result = get_eaf_data(
            self.simple_data, eval_min=1, eval_max=5, scale_eval_log=False
        )
        self.assertIsInstance(result, pd.DataFrame)
    ...
    def test_dual_runs_predictable_eaf(self):
        result = get_eaf_data(
            self.dual_data, eval_min=1, eval_max=3, scale_eval_log=False
        )
...
class TestGetEAFParetoData(unittest.TestCase):
    ...
    def test_return_types(self):
        ...
        # Polars return
        result_pl = get_eaf_pareto_data(
            self.simple_mo_data, "obj1", "obj2", return_as_pandas=False
        )
        self.assertIsInstance(result_pl, pl.DataFrame)
...
class TestGetEAFDiffData(unittest.TestCase):
    ...
    def test_identical_datasets_zero_diff(self):
        ...
        result = get_eaf_diff_data(
            self.identical_data1, self.identical_data2, "obj1", "obj2"
        )
        self.assertIsInstance(result, pd.DataFrame)
        self.assertIn("eaf_diff", result.columns)
    ...
    def test_return_types(self):
        ...
        # Polars return
        result_pl = get_eaf_diff_data(
            self.better_data, self.worse_data, "obj1", "obj2", return_as_pandas=False
        )
        self.assertIsInstance(result_pl, pl.DataFrame)
